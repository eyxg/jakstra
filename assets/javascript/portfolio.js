...
$(document).ready(() => {
  ...
  app.titleChanger(['jak', 'stra', '1337']);
...
$.getJSON(ipgeolocation, (data) => {
  writeLine(['Authenticating...', "Granting access to <span style='font-size: 14px; color: #06d;'>[jakstra1337.xyz]</span>..."], 30, () => {
    ...
    writeLine([`Access accepted! <span style='font-size: 14px; color: #0f0;'>[success]</span>`, `Hey user, <i style='color: #0f0'>${ip}</i>! By the way, nice to see someone from ${country} here!`], 30, 500, () => {
      if (app.skippedIntro) return;

      clearCursor();

      writeLine([`<i style='color: #F62459'>$$$ www.jakstra1337.xyz $_$</i>`], 120, 500, () => {
        timeouts.push(
          setTimeout(() => {
            if (app.skippedIntro) return;
